...
const pixelImageBuilder = retropixels.GraphicModes[options.mode]
...
const palette = retropixels.Palettes[options.palette]
...
const colorspace = retropixels.ColorSpaces[options.colorspace]
...
function saveExecutable (pixelImage, outFile) {
  const binary = retropixels.toBinary(pixelImage)
  ...
  const buffer = retropixels.toBuffer(binary)
...
retropixels.readImage(inFile, pixelImage.mode, options.scale)
  .then(async jimpImage => {
    ...
    if (!options.format) {
      ...
      await checkOverwrite(outFile)
      await retropixels.writeImage(pixelImage, outFile, palette)
    }
