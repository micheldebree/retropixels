...
import { ImageData } from './src/model/ImageData';
import { Converter } from './src/conversion/Converter';
import { PixelImage } from './src/model/PixelImage';
import { c64Multicolor } from './src/profiles/GraphicModes';

const graphicMode = c64Multicolor
...
function savePrg(pixelImage: PixelImage): void {
    const
        koalaImage: KoalaPicture = KoalaPicture.fromPixelImage(pixelImage),
    ...
    fs.readFile(binary, function(err, viewerCode) {
        ...
        const
            koalaBuffer: Buffer = new Buffer(koalaImage.toBytes()),
...
function cropFill(jimpImage: any, relativeWidth: number, relativeHeight: number): void {
    const
        srcWidth: number = jimpImage.bitmap.width,
...
jimp.read(inFile, (err: Error, jimpImage: any) => {
    ...
    const converter = new Converter(graphicMode);
