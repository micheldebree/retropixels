...
export class ImageData {
    ...
    static poke(imageData: ImageDataInterface, x: number, y: number, pixel: number[]): void {
        if (pixel !== undefined) {
    ...
    static peek(imageData: ImageDataInterface, x: number, y: number): number[] {
        const i: number = this.coordsToindex(imageData, x, y);
        if (i !== undefined) {
            return [
                imageData.data[i],
    ...
    static drawImageData(imageData: ImageDataInterface, pixelImage: PixelImage) {
        for (let y: number = 0; y < pixelImage.height; y += 1) {
            for (let x: number = 0; x < pixelImage.width; x += 1) {
                let pixel: number[] = this.peek(imageData, x, y);
