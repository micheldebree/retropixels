...
export default class ColorSpaces {
  // RGB is a noop (assume input is RGB)
  public static rgb: ColorSpace = (pixel: number[]): number[] => {
    return pixel;
  };

  public static rainbow: ColorSpace = (pixel: number[]): number[] => {
    return [ColorSpaces.yuv(pixel)[0], 0, 0];
  };

  // SDTV with BT.602
  // https://en.wikipedia.org/wiki/YUV
  public static yuv: ColorSpace = (pixel: number[]): number[] => {
    return [
  ...
  public static yCbCr: ColorSpace = (pixel: number[]): number[] => {
    const [r, g, b] = pixel;
    return [
  ...
  public static xyz: ColorSpace = (pixel: number[]): number[] => {
    let r = pixel[0];
    let g = pixel[1];
    let b = pixel[2];
  ...
  public static lab: ColorSpace = (pixel: number[]): number[] => {
    const xyzPixel = ColorSpaces.xyz(pixel);
    const refX = 95.047;
    const refY = 100.0;
    const refZ = 108.883;
    const pow = 1 / 3;
  ...
  public static okLab: ColorSpace = (pixel: number[]): number[] => {
